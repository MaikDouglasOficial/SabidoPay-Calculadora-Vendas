...
const JurosCalculatorScreen = () => {
    ...
    const performCalculation = () => {
        ...
        const previousValue = parseFloat(calcPreviousValue ?? '0');
    ...
    const handleCalcEquals = () => {
        ...
        setCalcWaitingForOperand(false);
        setCalcDisplay(String(resultValue));
    };
    ...
    const calcularParcela = () => {
        ...
        try {
            ...
            if (isNaN(parsedValue)) throw new Error("Valor inválido.");
            valorInicial = Math.max(0, parsedValue);
        }
        catch (error: any) {
            setLoading(false);
            Alert.alert("Erro", error?.message || "Erro ao ler valor.");
            setResultado([]);
            return;
        }
        if (valorInicial <= 0) {
            if (valorProduto !== '0') Alert.alert("Atenção", "Valor zerado ou inválido.");
            setResultado([]);
            setLoading(false);
            return;
        }
        const resultados: ResultadoParcela[] = [];
        const entrada = temEntrada ? Math.max(0, parseFloat(valorEntrada.replace(',', '.')) || 0) : 0;
        const formatCurrency = (num: number) => `R$${num.toFixed(2).replace('.', ',')}`;
        Object.keys(parcelas).forEach((key) => {
            const parcelaInfo = parcelas[key];
            const qtdParcelas = parseInt(key.split('+')[1].replace('X', ''));
            const jurosAplicado = selectedTabela === 'c' ? parcelaInfo.jrdia : parcelaInfo.pmeta;
            const valorBaseParaCalculo = valorInicial * (1 + jurosAplicado / 100);
            const valorRestante = valorBaseParaCalculo - entrada;
            if (valorRestante > 1e-6) {
                const valorParcelado = valorRestante / qtdParcelas;
                if (!temEntrada || entrada >= (valorParcelado - 1e-6)) {
                    resultados.push({ entrada: formatCurrency(entrada), parcelas: `${qtdParcelas}x`, valor: formatCurrency(valorParcelado) });
                }
            }
        });
            ...
            <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
                <View style={styles.calculatorContainer}>
                    ...
                    <View style={styles.calculatorDisplayContainer}>
                        {calcPreviousValue !== null && calcOperator !== null && (
                            <Text style={styles.calculatorPreviousValue}>
                                {calcPreviousValue} {calcOperator}
                            </Text>
                        )}
